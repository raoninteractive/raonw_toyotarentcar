...
<?
	...
	$sql = "
			SELECT * FROM (
				SELECT '1' AS gubun, '네이버(naver.com)' AS site_name, COUNT(*) AS total_cnt FROM statistics WHERE DATEDIFF(reg_date, '$search_sdate')>=0 AND DATEDIFF(reg_date, '$search_edate')<=0 AND referrer_url LIKE '%naver%'
				UNION ALL
				SELECT '2' AS gubun, '다 음(daum.net)' AS site_name, COUNT(*) AS total_cnt FROM statistics WHERE DATEDIFF(reg_date, '$search_sdate')>=0 AND DATEDIFF(reg_date, '$search_edate')<=0 AND referrer_url LIKE '%daum%'
				UNION ALL
				SELECT '3' AS gubun, '네이트(nate.com)' AS site_name, COUNT(*) AS total_cnt FROM statistics WHERE DATEDIFF(reg_date, '$search_sdate')>=0 AND DATEDIFF(reg_date, '$search_edate')<=0 AND referrer_url LIKE '%nate%'
				UNION ALL
				SELECT '4' AS gubun, '구 글(google.co.kr)' AS site_name, COUNT(*) AS total_cnt FROM statistics WHERE DATEDIFF(reg_date, '$search_sdate')>=0 AND DATEDIFF(reg_date, '$search_edate')<=0 AND referrer_url LIKE '%google%'
				UNION ALL
				SELECT '5' AS gubun, '기타' AS site_name, COUNT(*) AS total_cnt FROM statistics WHERE DATEDIFF(reg_date, '$search_sdate')>=0 AND DATEDIFF(reg_date, '$search_edate')<=0 AND NOT(referrer_url LIKE '%naver%' OR referrer_url LIKE '%daum%' OR referrer_url LIKE '%nate%' OR referrer_url LIKE '%google%')
			) t
			ORDER BY gubun
		";
			...
			<div class="common_list">
				<nav class="list_category five">
					<ul>
						...
						<li><span><a href="statistics_visit_week.php?page=<?=$page . $page_params?>"><span>요일별 방문자수</span></a></li>
						<li class="curr"><span><a href="statistics_visit_route.php?page=<?=$page . $page_params?>"><span>방문경로</span></a></li>
